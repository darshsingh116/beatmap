        ...
        if h_ptr<len(beatmap_data):
            ...
            if (hitobj[2] == a[0]) or (a[0]-hitobj[2] > 0):
                ...
                if 1 in indices_of_ones:
                    if hitobj[17] == 2:
                        # current[1] = hitobj[18]-prevTime
                        prevIsSpinnerOrSliderEndTimestamp = int(hitobj[18])
                        processed_beatmap.append([a[0],int(hitobj[3]),2])
                    elif hitobj[17] == 1:
                        prevIsSpinnerOrSliderEndTimestamp = int(hitobj[18])
                        processed_beatmap.append([a[0],int(hitobj[3]),1])    
                    else:
                        prevIsSpinnerOrSliderEndTimestamp = int(hitobj[18])
                        processed_beatmap.append([a[0],int(hitobj[3]),0])          
                ...
                if prevIsSpinnerOrSliderEndTimestamp == -1:
                    ...
                    if ((a[0] <= prevIsSpinnerOrSliderEndTimestamp) or (abs(a[0] - prevIsSpinnerOrSliderEndTimestamp)<10)):
                        binary_representation = bin(last_hit_type)[-4:]
                        # Check the last three bits
                        last_three_bits = binary_representation.zfill(3)  # Ensure it's always 3 bits long
                        # Find indices of '1' bits
                        indices_of_ones = [i for i, bit in enumerate(reversed(last_three_bits)) if bit == '1']
                        if 3 in indices_of_ones:
                            if (hitobj[2] == a[0]) or (abs(a[0] - prevIsSpinnerOrSliderEndTimestamp)<10):
                                processed_beatmap.append([a[0],hit_type,2])
                                prevIsSpinnerOrSliderEndTimestamp = -1
                            else:
                                processed_beatmap.append([a[0],hit_type,1])
                        else:
                            if processed_beatmap[-1][2] == 2:
                                processed_beatmap[-1][2] = 1
